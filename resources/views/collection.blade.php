...
<body>
	...
	<div class="img_1 zoom" >
		...
		<form class="" action="collections" method="get">

			<button id="btnA" class="titreCollec" name="descriptionCollec" value="{{$collection->collection_id}}" ">
				<i class="titreCollection"><span>{{$collection->collection_nom}}</span></i>
				<img class="img_1 zoom"  src="{{$collection->collection_lien_image}}"></button>
			</form>
		</div>
	@endforeach


	@if (isset($_GET['descriptionCollec']))
		<h3 class="titre"><span> Description </span></h3>
		<p class="description">{{$collection->bd_id}}</p>
